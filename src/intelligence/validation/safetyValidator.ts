...
export class SafetyValidator {
  private constraints: Map<string, OperationConstraint> = new Map();
  private riskHistory: Array<{ timestamp: number; riskLevel: string }> = [];
  ...
  private initializeDefaultConstraints(): void {
    ...
    this.addConstraint({
      ...
      rule: (context) => {
        ...
        return required.every((perm: string) => available.includes(perm));
      },
      severity: 'error',
  ...
  public addConstraint(constraint: OperationConstraint): void {
    this.constraints.set(constraint.name, constraint);
  }

  public async validateOperation(context: Record<string, any>): Promise<ValidationResult> {
    const results: ValidationResult['constraints'] = [];
  ...
  public getConstraints(): Record<string, OperationConstraint> {
    const result: Record<string, OperationConstraint> = {};
