export interface ExecutionThresholds {
  executionThreshold: number; // confidence > 0.85 → execute immediately
  rollbackThreshold: number; // confidence > 0.70 → execute with rollback ready
  confirmationThreshold: number; // confidence < 0.70 → ask user
  blockingThreshold: number; // confidence < 0.30 → reject execution
}
...
export default class AutonomyController {
  ...
  private confidenceCalibration: Map<string, { predicted: number; actual: number }[]> =
    new Map();

  /**
   * Decide whether to execute a request autonomously
   */
  public decide(request: ExecutionRequest): ExecutionDecision {
    const confidence = request.prediction.confidence;

    // Apply risk adjustment
    const adjustedConfidence = this.adjustConfidenceForRisk(
      confidence,
      request.prediction.riskLevel,
      request.context
    );
  ...
  public async executeDecision(
    ...
    decision: ExecutionDecision,
    executor?: (req: ExecutionRequest) => Promise<{ success: boolean; output: Record<string, unknown> }>
  ): Promise<ExecutionOutcome> {
    const startTime = Date.now();
    let executed = false;
    let result: ExecutionOutcome['result'] = undefined;
    let reason = '';
   ...
   */
  private getThresholdUsed(confidence: number): keyof ExecutionThresholds {
   ...
   */
  public calibrateThresholds(): void {
    // Analyze confidence calibration
    let totalDrift = 0;
    let calibrationPoints = 0;

    for (const [action, calibrations] of this.confidenceCalibration.entries()) {
      if (calibrations.length < 5) continue; // Need at least 5 samples

      const avgPredicted =
        calibrations.reduce((sum, c) => sum + c.predicted, 0) / calibrations.length;
      const avgActual =
        calibrations.reduce((sum, c) => sum + c.actual, 0) / calibrations.length;
    ...
    if (avgDrift < -0.1) {
      ...
      this.thresholds.executionThreshold = Math.min(
        0.95,
        this.thresholds.executionThreshold + 0.02
      );
      this.thresholds.rollbackThreshold = Math.min(
        0.85,
        this.thresholds.rollbackThreshold + 0.02
      );
    } else if (avgDrift > 0.1) {
      // Too pessimistic, be more aggressive
      this.thresholds.executionThreshold = Math.max(
        0.75,
        this.thresholds.executionThreshold - 0.02
      );
      this.thresholds.rollbackThreshold = Math.max(
        0.6,
        this.thresholds.rollbackThreshold - 0.02
      );
    }
    ...
    console.log(
      `[v0] Calibrated autonomy thresholds based on drift: ${avgDrift.toFixed(3)}`
    );
  }
  ...
  public getAutonomyMetrics(): {
    ...
    const autonomous = this.executionHistory.filter((o) =>
      ['execute', 'execute-with-rollback'].includes(o.decision)
    ).length;
    const executed = this.executionHistory.filter((o) => o.executed).length;
    const successful = this.executionHistory.filter(
      (o) => o.result && o.result.success
    ).length;
   ...
   */
  private getAverageConfidenceForOutcome(success: boolean): number {
    ...
    return (
      outcomes.reduce((sum, o) => sum + (confidenceMap[o.decision] || 0.5), 0) /
      outcomes.length
    );
  }
   ...
   */
  public getHistory(): ExecutionOutcome[] {
   ...
   */
  public getThresholds(): ExecutionThresholds {
   ...
   */
  public setThresholds(thresholds: Partial<ExecutionThresholds>): void {
