...
export default class GoalDecompositionEngine {
   ...
   */
  public decompose(userGoal: string): GoalTree {
    const rootGoal = this.createGoal(
      userGoal,
      'High-level user goal',
      this.analyzeComplexity(userGoal),
      []
    );
    ...
    // Calculate totals
    const totalEffort = Array.from(allGoals.values()).reduce(
      (sum, g) => sum + g.estimatedEffort,
      0
    );
   ...
   */
  private generateSubGoals(parentGoal: Goal, depth: number): Goal[] {
    const subGoals: Goal[] = [];
    ...
    if (depth === 0) {
      ...
      subGoals.push(
        this.createGoal(
          `Analyze: ${parentGoal.title}`,
          'Understand requirements and dependencies',
          'simple',
          ['analysis', parentGoal.tags[0] || 'general']
        )
      );
      subGoals.push(
        this.createGoal(
          ...
          'simple',
          ['planning', parentGoal.tags[0] || 'general']
        )
      );
      subGoals.push(
        this.createGoal(
          `Execute: ${parentGoal.title}`,
          'Implement the actual changes',
          parentGoal.complexity === 'complex' ? 'moderate' : 'simple',
          ['execution', parentGoal.tags[0] || 'general']
        )
      );
      subGoals.push(
        this.createGoal(
          `Verify: ${parentGoal.title}`,
          'Test and validate results',
          'simple',
          ['verification', parentGoal.tags[0] || 'general']
        )
      );
    } else if (depth === 1) {
      ...
      if (parentGoal.title.includes('Analyze')) {
        subGoals.push(
          ...
          this.createGoal(
            'Check constraints',
            'Verify limitations and blockers',
            'atomic',
            ['analysis']
          )
        );
        ...
        subGoals.push(
          this.createGoal(
            `Step 1: ${parentGoal.title}`,
            'First phase',
            'atomic',
            parentGoal.tags
          ),
          this.createGoal(
            `Step 2: ${parentGoal.title}`,
            'Second phase',
            'atomic',
            parentGoal.tags
          )
   ...
   */
  private analyzeComplexity(goal: string): 'atomic' | 'simple' | 'moderate' | 'complex' {
    const goalLower = goal.toLowerCase();
    let complexity: 'atomic' | 'simple' | 'moderate' | 'complex' = 'simple';

    // Complexity signals
    if (
      goalLower.includes('refactor') ||
    ...
    if (operationCount > 2 && complexity === 'atomic') complexity = 'simple';
    if (operationCount > 3 && complexity === 'simple') complexity = 'moderate';
   ...
   */
  private determineComplexity(effort: number): 'atomic' | 'simple' | 'moderate' | 'complex' {
    if (effort <= this.complexityThresholds.atomic) return 'atomic';
    if (effort <= this.complexityThresholds.simple) return 'simple';
    if (effort <= this.complexityThresholds.moderate) return 'moderate';
    return 'complex';
  }
   ...
   */
  private topologicalSort(goals: Map<string, Goal>): string[] {
   ...
   */
  private findCriticalPath(goals: Map<string, Goal>, executionOrder: string[]): string[] {
    ...
    for (const goalId of executionOrder.reverse()) {
      ...
      if (goal.children.length === 0) {
        ...
        const maxChildLength = Math.max(
          ...goal.children.map((childId) => pathLengths.get(childId) || 0)
        );
    ...
    while (current) {
      ...
      const nextChild = goal.children.find(
        (childId) =>
          (pathLengths.get(childId) || 0) ===
   ...
   * Get goal by ID
   */
  public getGoal(tree: GoalTree, goalId: string): Goal | undefined {
   ...
   */
  public markCompleted(tree: GoalTree, goalId: string): void {
   ...
   * Get remaining goals
   */
  public getRemainingGoals(tree: GoalTree): Goal[] {
    return Array.from(tree.allGoals.values()).filter(
      (g) => !g.tags.includes('completed')
    );
  }
   ...
   */
  public getProgress(tree: GoalTree): {
    completed: number;
    total: number;
    percentage: number;
  } {
    const total = tree.allGoals.size;
    const completed = Array.from(tree.allGoals.values()).filter((g) =>
