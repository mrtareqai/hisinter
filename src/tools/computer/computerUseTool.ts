...
export function createComputerUseTools(ssDir: string): ToolDefinition[] {
    ...
    return [
        // ── Screenshot ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                ...
                const r = args.region as string | undefined;
                ...
                } catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Mouse Click ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                const x = args.x, y = args.y, btn = (args.button as string) || 'left';
                const n = parseInt((args.clicks as string) || '1');
        ...
        // ── Mouse Move ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                try { await runPS(`[NM]::SetCursorPos(${args.x},${args.y})`); return { success: true, output: `Moved to (${args.x},${args.y})` }; }
                catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Mouse Scroll ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                const amt = parseInt((args.amount as string) || '3');
                const delta = args.direction === 'up' ? 120 * amt : -120 * amt;
                try { await runPS(`[NM]::mouse_event([uint32][NM]::WH,0,0,${delta},0)`); return { success: true, output: `Scrolled ${args.direction} x${amt}` }; }
                catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Keyboard Type ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                const t = (args.text as string).replace(/'/g, "''").replace(/[+^%~(){}[\]]/g, '{$&}');
                try { await runPS(`[Windows.Forms.SendKeys]::SendWait('${t}')`, 15000); return { success: true, output: `Typed: "${args.text}"` }; }
                catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Keyboard Hotkey ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                const keyMap: Record<string, string> = { enter: '{ENTER}', tab: '{TAB}', escape: '{ESC}', esc: '{ESC}', backspace: '{BACKSPACE}', delete: '{DELETE}', up: '{UP}', down: '{DOWN}', left: '{LEFT}', right: '{RIGHT}', home: '{HOME}', end: '{END}', f1: '{F1}', f2: '{F2}', f3: '{F3}', f4: '{F4}', f5: '{F5}', f6: '{F6}', f7: '{F7}', f8: '{F8}', f9: '{F9}', f10: '{F10}', f11: '{F11}', f12: '{F12}', space: ' ' };
                const parts = (args.keys as string).toLowerCase().split('+').map(k => k.trim());
                let mods = '', key = '';
                ...
                try { await runPS(`[Windows.Forms.SendKeys]::SendWait('${mods}${key}')`); return { success: true, output: `Pressed: ${args.keys}` }; }
                catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Screen Info ──
        ...
        // ── Open App ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                try { await runPS(`Start-Process '${(args.target as string).replace(/'/g, "''")}'`); return { success: true, output: `Opened: ${args.target}` }; }
                catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Wait ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                const ms = Math.min(parseInt(args.ms as string) || 1000, 30000);
        ...
        // ── Clipboard Set ──
        {
            ...
            execute: async (args): Promise<ToolResult> => {
                try {
                    const t = (args.text as string).replace(/'/g, "''");
                    await runPS(`Set-Clipboard -Value '${t}'`);
                    return { success: true, output: `Copied to clipboard: "${(args.text as string).substring(0, 50)}..."` };
                } catch (e) { return { success: false, output: `Failed: ${(e as Error).message}` }; }
            },
        },
        // ── Real OCR ──
            ...
            execute: async (args): Promise<ToolResult> => {
                ...
                try {
                    ...
                    try {
                        ...
                        const searchTerm = args.search as string;
                        if (searchTerm) {
