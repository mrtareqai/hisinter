...
import { GroqProviderConfig, GroqApiKey } from '../ai/groqClient';
...
export type RuntimeExecutionState =
    'idle' | 'planning' | 'executing' | 'validating' | 'reflecting' | 'retrying' | 'completed' | 'failed' | 'paused';
...
export class HybridModelRuntime {
    private config: HybridRuntimeConfig;
    private discoveredModels: RuntimeModelDescriptor[] = [];
    ...
    getActiveSelection(modeHint?: string): { model: string; provider: string; location: RuntimeLocation; contextWindow: number } {
        const hasCloud = this.hasCloudKeys();
        const wantsCloud =
            (this.config.cloud120BEnabled && hasCloud) ||
            this.config.routeMode === 'cloud-120b' ||
    ...
    setDiscoveredModels(models: RegisteredModel[]): void {
        ...
        const byId = new Map<string, RuntimeModelDescriptor>();
    ...
    private buildGroqProvider(priority: number): ProviderEntry | null {
    ...
    private fromRegisteredModel(model: RegisteredModel): RuntimeModelDescriptor {
        const location: RuntimeLocation = model.isOnline || model.providerType === 'groq' ? 'cloud' : 'local';
    ...
    private detectRoles(modelName: string, vision: boolean): string[] {
    ...
    private is120BModel(modelName: string): boolean {
        const lower = modelName.toLowerCase();
        return lower.includes('120b') || lower.includes('gpt-oss');
    }

    private reconcileLocalSelectionWithInstalledModels(discoveredDescriptors: RuntimeModelDescriptor[]): void {
        ...
        const configuredExists = localModels.some((model) =>
            model.name.toLowerCase() === configured || model.id.toLowerCase() === this.localModelId(this.config.localModel).toLowerCase()
        );
        if (configuredExists) {
            return;
        }
        ...
        const preferred =
            localModels.find((model) => model.name.toLowerCase().includes('deepseek-coder')) ||
    ...
    private localModelId(model: string): string {
        return `ollama/${model}`;
    }

    private cloudModelId(model: string): string {
        return `groq/${model}`;
    }
}
