...
import { ModelAdapter, ModelCapabilities, AdaptedResponse, ProviderEntry } from '../ai/modelAdapter';
...
import { AutonomousRuntimeState, GoalStateSnapshot } from '../runtime/autonomousRuntimeState';
import { ContextCompressor } from '../runtime/contextCompressor';
import { ValidationReport, ValidationRunner } from '../runtime/validationRunner';
import { getCompactWorkspaceMap, invalidateWorkspaceMap } from '../context/workspaceMap';
...
interface SlashCommand {
    name: string;
    description: string;
    handler: (args: string, agent: AgentLoop) => AsyncGenerator<AgentEvent>;
}
...
export class AgentLoop {
    ...
    private runtime = new AutonomousRuntimeState();
    private compressor = new ContextCompressor();
    private validationRunner: ValidationRunner | null = null;
    ...
    private taskLedger: {
        ...
        lastAction: string;
        stepCount: number;
    } = { goal: '', filesTouched: [], lastAction: '', stepCount: 0 };
    ...
    constructor(tools: ToolRegistry, config: Partial<AgentConfig> = {}) {
        ...
        this.client = new OllamaClient(this.config.ollamaUrl);
        this.adapter = new ModelAdapter(this.client);
        this.tools = tools;
    }
    ...
    setModel(model: string): void { this.config.model = model; this.adapter.clearCache(); }
    setSystemPrompt(prompt: string): void { this.config.systemPrompt = prompt; }
    getHistory(): OllamaMessage[] { return [...this.conversationHistory]; }
    getModel(): string { return this.config.model; }
    getAdapter(): ModelAdapter { return this.adapter; }
    setValidationEnabled(enabled: boolean): void {
        this.validationEnabled = enabled;
        if (this.workspaceRoot) {
            this.validationRunner = new ValidationRunner(this.workspaceRoot, enabled);
        }
    }
    getRuntimeState(): GoalStateSnapshot { return this.runtime.snapshot(); }
    ...
    getProviderInfo(): { provider: string; model: string; isOnline: boolean } {
        return {
    ...
    setWorkspaceRoot(root: string): void {
        ...
        this.validationRunner = new ValidationRunner(root, this.validationEnabled);
    }
    ...
    private getSystemPrompt(): string {
        ...
        if (!prompt) {
            switch (this.mode) {
                case 'agent': prompt = AGENT_PROMPT; break;
                case 'computer': prompt = COMPUTER_PROMPT; break;
                case 'max': prompt = MAX_PROMPT; break;
                case 'deep': prompt = DEEP_PROMPT; break;
                case 'planning': prompt = PLANNING_PROMPT; break;
                case 'debug': prompt = DEBUG_PROMPT; break;
                case 'monitor': prompt = MONITOR_PROMPT; break;
                default: prompt = CHAT_PROMPT; break;
            }
        }
        ...
        if (this.workspaceRoot && this.mode !== 'chat') {
            try {
                prompt += '\n' + getCompactWorkspaceMap(this.workspaceRoot);
    ...
    private isActionRequest(text: string): boolean {
        ...
        if (this.isSimpleGreeting(lower)) { return false; }
    ...
    // ─── Deep Analysis Mode ─────────────────────────────────
    private isProjectBootstrapRequest(message: string): boolean {
    ...
    private async *runDirectProjectBootstrap(userMessage: string): AsyncGenerator<AgentEvent> {
    ...
    private inferProjectName(message: string): string {
        ...
        if (lower.includes('dashboard')) { return 'sinter-dashboard'; }
        if (lower.includes('api')) { return 'sinter-api'; }
        if (lower.includes('landing')) { return 'sinter-landing'; }
    ...
    private assessRequestConfidence(message: string): number {
        const lower = message.toLowerCase();
        let score = 0;
        // Has a project type mentioned?
        if (['react', 'next', 'vue', 'angular', 'fastapi', 'django', 'flask', 'express', 'html', 'landing'].some(t => lower.includes(t))) { score += 0.3; }
        // Has a name?
        if (/(?:called|named|اسم)\s+\S+/i.test(lower)) { score += 0.2; }
        // Has descriptive details (more than 8 words)?
        if (lower.split(/\s+/).length > 8) { score += 0.2; }
        // Has feature keywords?
        if (['login', 'dashboard', 'api', 'form', 'database', 'auth', 'portfolio', 'blog', 'shop', 'store', 'chat', 'dark', 'responsive'].some(f => lower.includes(f))) { score += 0.2; }
        // Basic action + artifact is minimum
        if (score === 0) { score = 0.1; }
        return Math.min(score, 1);
    }

    private async *askClarification(userMessage: string): AsyncGenerator<AgentEvent> {
        const questions = [
        ...
        if (this.memory) { this.memory.addMessage('assistant', questions); }
        yield { type: 'response', content: questions };
    }

    private shouldUseDeepMode(message: string): boolean {
        if (this.mode !== 'agent' && this.mode !== 'max' && this.mode !== 'debug') { return false; }
        const deepKeywords = [
    ...
    private async *runDeepMode(userMessage: string): AsyncGenerator<AgentEvent> {
        const reporter = new ProgressReporter();
        const explorer = new CodebaseExplorer();
        const pipeline = new AnalysisPipeline();
        const planner = new PlanBuilder();
        ...
        try {
            ...
            yield reporter.foundStats(
                projectMap.stats.totalSourceFiles,
                projectMap.stats.totalLines,
            ...
            const summaryResponse = await this.adapter.chat(
                this.config.model, summaryMessages, [], { temperature: 0.3 }
            );
            ...
            yield reporter.error(`Deep analysis failed: ${(e as Error).message}`);
    ...
    private async *handleSlashCommand(cmd: string, args: string): AsyncGenerator<AgentEvent> {
        switch (cmd) {
            case 'help':
            ...
            case 'screenshot':
                ...
                try {
                    ...
                    yield { type: 'error', content: `Screenshot failed: ${(e as Error).message}` };
                }
                return;

            case 'run':
                if (!args) { yield { type: 'error', content: 'Usage: /run <command>' }; return; }
                yield { type: 'info', content: `💻 Running: ${args}` };
                try {
                    const result = await this.tools.execute('terminal_run', { command: args });
                    yield { type: 'tool_result', content: result.success ? `✅ ${result.output}` : `❌ ${result.output}` };
                } catch (e) {
                    yield { type: 'error', content: `Command failed: ${(e as Error).message}` };
                }
                return;

            case 'search':
                if (!args) { yield { type: 'error', content: 'Usage: /search <query>' }; return; }
                yield { type: 'info', content: `🔍 Searching: ${args}` };
                try {
                    const result = await this.tools.execute('browser_search', { query: args });
                    yield { type: 'tool_result', content: result.success ? `✅ ${result.output}` : `❌ ${result.output}` };
                } catch (e) {
                    yield { type: 'error', content: `Search failed: ${(e as Error).message}` };
                }
                return;

            case 'open':
                if (!args) { yield { type: 'error', content: 'Usage: /open <url or app>' }; return; }
                ...
                try {
                    ...
                    yield { type: 'error', content: `Open failed: ${(e as Error).message}` };
                }
                return;

            case 'file':
                if (!args) { yield { type: 'error', content: 'Usage: /file read <path> or /file list <dir>' }; return; }
                ...
                try {
                    ...
                    yield { type: 'error', content: `File operation failed: ${(e as Error).message}` };
                }
                return;

            case 'code':
                if (!args) { yield { type: 'error', content: 'Usage: /code <description>' }; return; }
                // Route to agent mode for code generation
                this.conversationHistory.push({ role: 'user', content: `Create code: ${args}` });
                if (this.memory) { this.memory.addMessage('user', `/code ${args}`); }
            ...
            case 'project':
                if (!args) { yield { type: 'error', content: 'Usage: /project <name or description>' }; return; }
                this.runtime.startGoal(args);
                this.conversationHistory.push({ role: 'user', content: `Create project: ${args}` });
                if (this.memory) { this.memory.addMessage('user', `/project ${args}`); }
            ...
            case 'fix':
                ...
                if (this.activeFile) {
                    ...
                    if (this.memory) { this.memory.addMessage('user', `/fix ${this.activeFile}`); }
                    const pm = this.mode;
                    this.mode = 'agent';
            ...
            case 'explain':
                if (this.activeFile) {
                    ...
                    if (this.memory) { this.memory.addMessage('user', `/explain ${this.activeFile}`); }
                    yield* this.runChatMode();
                } else {
                    yield { type: 'error', content: '⚠️ No active file. Open a file first.' };
                }
...
⚡ Max — Auto-detect best mode`;
    ...
    // ─── Chat Mode ──────────────────────────────────────────
    private async *runChatMode(): AsyncGenerator<AgentEvent> {
        if (this.cancelled) { yield { type: 'info', content: '⏹️ Stopped' }; return; }
        ...
        try {
            ...
            const adapted = await this.adapter.chatWithFallback(
                this.config.model, messages,
                shouldPassTools ? this.tools.toOllamaTools() : [],
                { temperature: 0.7, num_predict: 2048 },
                (msg) => { fallbackEvents.push({ type: 'info', content: msg }); },
            );

            for (const fe of fallbackEvents) yield fe;
            ...
            if (adapted.isToolResponse && adapted.toolCalls.length > 0) {
                ...
                // Final summary
                const summary = await this.adapter.chatWithFallback(
                    this.config.model, [{ role: 'system', content: this.getSystemPrompt() }, ...this.getRecentHistory(10)],
                    [], { temperature: 0.3 }
                );
                yield { type: 'response', content: summary.content };
                if (this.memory) this.memory.addMessage('assistant', summary.content);
            } else {
                this.conversationHistory.push({ role: 'assistant', content: adapted.content });
                if (this.memory) { this.memory.addMessage('assistant', adapted.content); }
    ...
    private async *runAgentMode(): AsyncGenerator<AgentEvent> {
            ...
            try {
                ...
                if (adapted.isToolResponse && adapted.toolCalls.length > 0) {
                    ...
                    for (const tc of adapted.toolCalls) {
                        ...
                        if (usedTools.includes(sig)) { yield { type: 'info', content: 'Skipping duplicate tool call' }; continue; }
                        usedTools.push(sig);

                        yield { type: 'tool_call', content: `${tc.name}(${JSON.stringify(tc.arguments)})`, toolName: tc.name };

                        let result: ToolResult;
                        try { result = await this.tools.execute(tc.name, tc.arguments); }
                        catch (e) { result = { success: false, output: `Error: ${(e as Error).message}` }; }

                        if (result.images?.length) { yield { type: 'images', content: 'Screenshot', images: result.images }; }
                        ...
                        if (this.memory) { this.memory.addMessage('assistant', `[Tool: ${tc.name}] ${result.success ? '✅' : '❌'} ${result.output.substring(0, 200)}`); }
                        if (result.success) {
                            successCount++;
                            if (this.isMutationTool(tc.name)) {
                ...
                this.conversationHistory.push({ role: 'assistant', content: final });
                if (this.memory) { this.memory.addMessage('assistant', final); }
    ...
    // ─── Generate Summary ─────────────────────────────────────
    private async *generateSummary(): AsyncGenerator<AgentEvent> {
        yield { type: 'thinking', content: 'Summarizing...' };
        try {
            const sr = await this.adapter.chatWithFallback(
                this.config.model,
                [
            ...
            if (this.memory) { this.memory.addMessage('assistant', s); }
        ...
        } catch { yield { type: 'response', content: 'Done.' }; }
    }

    private async *runValidationAfterMutation(signature: string): AsyncGenerator<AgentEvent> {
        ...
        this.runtime.recordFailure(
            ...
            failed?.output.substring(0, 500) || report.summary,
            'read validation output, patch the exact failing owner, then rerun validation'
        );
    ...
    private formatValidationSummary(report: ValidationReport): string {
        if (report.skipped) {
            return report.summary;
        }
        const rows = report.checks.map((check) =>
            `${check.success ? 'PASS' : 'FAIL'} ${check.name} (${Math.round(check.durationMs / 1000)}s): ${check.command}`
        );
        return `${report.summary}\n${rows.join('\n')}`;
    }

    private formatRuntimeMonitor(): string {
    ...
    private isVisionLoop(tools: string[]): boolean {
    ...
    private getRecentHistory(max: number): OllamaMessage[] {
        return this.compressor.compress(this.conversationHistory, max, 28000).messages;
    }

    private async *handleConnectionError(errorDetail: string): AsyncGenerator<AgentEvent> {
    ...
    clearHistory(): void {
        ...
        if (this.memory) { this.memory.newConversation(this.mode, this.config.model); }
    }
    ...
    loadConversation(id: string): boolean {
        ...
        if (conv) {
            this.conversationHistory = conv.messages.map(m => ({ role: m.role as OllamaMessage['role'], content: m.content }));
            return true;
        }
        return false;
    }
