...
class ChatPanelProvider {
    ...
    modelRegistry;
    onRuntimeChanged;
    static viewType = 'sinter.chatPanel';
    webviewView;
    memory = null;
    contextProvider = null;
    workspaceStatsCache;
    constructor(extensionUri, agent, runtime, modelRegistry, onRuntimeChanged) {
    ...
    setContextProvider(contextProvider) { this.contextProvider = contextProvider; }
    resolveWebviewView(view) {
        ...
        view.webview.onDidReceiveMessage(async (msg) => {
            try {
                ...
                this.postMessage({ type: 'agentEvent', event: { type: 'error', content: `UI message failed: ${error.message}` } });
                this.postMessage({ type: 'agentDone' });
                this.sendRuntimeStatus();
            }
        });
    ...
    async handleSend(msg) {
        ...
        if (!finalMessage && !msg.contextItems?.length) {
            this.postMessage({ type: 'agentEvent', event: { type: 'info', content: 'Type a request first.' } });
            return;
        }
    ...
    async handleMessage(text, displayMessage, mode) {
        ...
        try {
            ...
            this.postMessage({ type: 'agentEvent', event: { type: 'error', content: `Fatal: ${error.message}` } });
        }
        this.postMessage({ type: 'agentDone' });
        this.sendRuntimeStatus();
    }
    ...
    async discoverModels() {
        ...
        try {
            ...
            this.postMessage({ type: 'agentEvent', event: { type: 'error', content: `Ollama Discovery Failed: ${e.message}` } });
        }
    }
    async selectModelFromPanel(modelId) {
        if (!modelId) {
            return;
        }
    ...
    resolveRequestedMode(text, requested) {
        const validModes = new Set(['chat', 'agent', 'computer', 'max', 'deep', 'planning', 'debug', 'monitor']);
        if (requested && requested !== 'auto' && validModes.has(requested)) {
            return requested;
        }
        return this.inferUnifiedMode(text);
    }
    inferUnifiedMode(text) {
    ...
    computeConfidence(status, taskCount) {
        let score = status.location === 'cloud' ? 92 : 82;
        if (status.agentState === 'max') {
            score += 7;
        }
        if (status.agentState === 'agent') {
            score += 5;
        }
        if (status.validationState === 'tracked') {
            score += 3;
        }
        if (status.executionState === 'failed') {
            score -= 25;
        }
        if (taskCount > 0 && status.goalProgress === 0 && status.toolState === 'busy') {
            score -= 4;
        }
    ...
    scanWorkspace(dir, stats, depth) {
        if (depth > 8 || stats.files > 5000) {
            return;
        }
        let entries;
        try {
