...
const fileEditTool_1 = require("./tools/fileEditTool");
const terminalTool_1 = require("./tools/terminalTool");
const browserTool_1 = require("./tools/browserTool");
const computerUseTool_1 = require("./tools/computer/computerUseTool");
const documentTool_1 = require("./tools/documentTool");
const astTool_1 = require("./tools/astTool");
const projectTool_1 = require("./tools/projectTool");
...
const hybridModelRuntime_1 = require("./runtime/hybridModelRuntime");
...
function activate(context) {
    ...
    try {
        ...
        const memory = new memoryStore_1.MemoryStore(memoryDir);
        const tools = new toolRegistry_1.ToolRegistry();
        const ollamaUrl = vscode.workspace.getConfiguration('sinter').get('ollamaUrl', 'http://localhost:11434');
        const modelRegistry = new modelRegistry_1.ModelRegistry();
        modelRegistry.updateOllamaProvider(ollamaUrl);
        const runtime = new hybridModelRuntime_1.HybridModelRuntime(readRuntimeConfig());
        const modelManager = new modelManager_1.ModelManagerProvider(ollamaUrl, modelRegistry);
        // Register tools
        for (const tool of (0, fileTool_1.createFileTools)(workspaceRoot)) {
            tools.register(tool);
        }
        for (const tool of (0, fileEditTool_1.createFileEditTools)(workspaceRoot)) {
            tools.register(tool);
        }
        for (const tool of (0, terminalTool_1.createTerminalTools)(workspaceRoot)) {
            tools.register(tool);
        }
        for (const tool of (0, browserTool_1.createBrowserTools)()) {
            tools.register(tool);
        }
        for (const tool of (0, documentTool_1.createDocumentTools)(workspaceRoot)) {
            tools.register(tool);
        }
        for (const tool of (0, astTool_1.createAstTools)(workspaceRoot)) {
            tools.register(tool);
        }
        for (const tool of (0, projectTool_1.createProjectTools)(workspaceRoot)) {
            tools.register(tool);
        }
        if (vscode.workspace.getConfiguration('sinter').get('computerUseEnabled', true)) {
            for (const tool of (0, computerUseTool_1.createComputerUseTools)(screenshotDir)) {
                tools.register(tool);
            }
        }
        const activeSelection = runtime.getActiveSelection();
        const agent = new agentLoop_1.AgentLoop(tools, {
            model: activeSelection.model,
            ollamaUrl: vscode.workspace.getConfiguration('sinter').get('ollamaUrl', 'http://localhost:11434'),
            systemPrompt: vscode.workspace.getConfiguration('sinter').get('systemPrompt', ''),
            maxIterations: 24,
        });
        agent.setMode(vscode.workspace.getConfiguration('sinter').get('defaultMode', 'chat'));
        agent.setMemory(memory);
        agent.setWorkspaceRoot(workspaceRoot);
        agent.setValidationEnabled(vscode.workspace.getConfiguration('sinter').get('validationEnabled', true));
        agent.setProviderChain(runtime.buildProviderChain(agent.getMode()));
        const contextProvider = new contextProvider_1.ContextProvider(workspaceRoot);
        ...
        const rebuildRuntime = async (discoverModels = false) => {
            try {
                ...
                agent.setProviderChain(runtime.buildProviderChain(agent.getMode()));
                agent.setValidationEnabled(config.get('validationEnabled', true));
        ...
        chatProvider = new chatPanel_1.ChatPanelProvider(context.extensionUri, agent, runtime, modelRegistry, rebuildRuntime);
        ...
        context.subscriptions.push(vscode.window.registerWebviewViewProvider(chatPanel_1.ChatPanelProvider.viewType, chatProvider), vscode.window.registerTreeDataProvider('sinter.modelManager', modelManager));
        ...
        context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(updateActiveFile), vscode.workspace.onDidSaveTextDocument(updateActiveFile));
        updateActiveFile();
        context.subscriptions.push(vscode.commands.registerCommand('sinter.newChat', () => {
            agent.clearHistory();
        ...
        }), vscode.commands.registerCommand('sinter.pullModel', () => modelManager.pullModel()), vscode.commands.registerCommand('sinter.toggle120BMode', async (enabled) => {
            const config = vscode.workspace.getConfiguration('sinter');
            const next = typeof enabled === 'boolean' ? enabled : !config.get('use120BMode', false);
        ...
        }), vscode.commands.registerCommand('sinter.rebuildChain', () => rebuildRuntime(true)), vscode.commands.registerCommand('sinter.addGroqKey', async () => {
            ...
            const config = vscode.workspace.getConfiguration('sinter');
            const keys = config.get('groqApiKeys', []);
            ...
            const model = await vscode.window.showInputBox({
                ...
                value: config.get('groqModel', 'openai/gpt-oss-120b'),
            });
            if (!model) {
                return;
            }
        ...
        }), vscode.commands.registerCommand('sinter.computerUse', () => setMode(agent, chatProvider, 'computer')), vscode.commands.registerCommand('sinter.agentMode', () => setMode(agent, chatProvider, 'agent')), vscode.commands.registerCommand('sinter.maxMode', () => setMode(agent, chatProvider, 'max')), vscode.commands.registerCommand('sinter.deepMode', () => setMode(agent, chatProvider, 'deep')), vscode.commands.registerCommand('sinter.sendFileToAgent', () => sendActiveFileToAgent(agent, chatProvider)), vscode.commands.registerCommand('sinter.fixProblems', () => sendDiagnosticsToAgent(agent, chatProvider)), vscode.commands.registerCommand('sinter.debugOllama', async () => {
            const url = vscode.workspace.getConfiguration('sinter').get('ollamaUrl', 'http://localhost:11434');
            const client = new ollamaClient_1.OllamaClient(url);
            try {
                ...
                vscode.window.showErrorMessage(`Ollama Error at ${url}: ${e.message}`);
            }
        }));
        context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(async (event) => {
            if (event.affectsConfiguration('sinter.activeModel') ||
            ...
            if (event.affectsConfiguration('sinter.systemPrompt')) {
                agent.setSystemPrompt(vscode.workspace.getConfiguration('sinter').get('systemPrompt', ''));
            }
        ...
        // Background tasks
        setTimeout(() => {
            new ollamaClient_1.OllamaClient(vscode.workspace.getConfiguration('sinter').get('ollamaUrl', 'http://localhost:11434'))
                .isRunning()
                .then((running) => {
                if (!running) {
...
function readRuntimeConfig() {
    const config = vscode.workspace.getConfiguration('sinter');
    const localDefault = config.get('localDefaultModel', 'deepseek-coder:6.7b');
    const activeModel = config.get('activeModel', localDefault) || localDefault;
    const use120B = config.get('use120BMode', false);
    const routeMode = config.get('runtimeRouteMode', use120B ? 'cloud-120b' : 'local');
    return {
        localModel: config.get('localDefaultModel', activeModel) || activeModel,
        cloudModel: config.get('groqModel', 'openai/gpt-oss-120b'),
        ollamaUrl: config.get('ollamaUrl', 'http://localhost:11434'),
        groqBaseUrl: config.get('groqBaseUrl', 'https://api.groq.com/openai'),
        groqApiKeys: config.get('groqApiKeys', []),
        temperature: config.get('temperature', 0.7),
        maxTokens: config.get('maxTokens', 8192),
        routeMode,
        selectedModelId: config.get('selectedRuntimeModel', '') || undefined,
        cloud120BEnabled: use120B,
    };
}
function loadConfiguredProviders(registry) {
    const config = vscode.workspace.getConfiguration('sinter');
    const providers = config.get('modelProviders', []);
    if (providers.length > 0) {
        registry.loadFromSettings(providers);
    }
    const groqKeys = config.get('groqApiKeys', []);
    registry.addProvider({
        id: 'groq',
        name: 'Groq Cloud',
        baseUrl: config.get('groqBaseUrl', 'https://api.groq.com/openai'),
        type: 'groq',
        apiKeys: groqKeys,
...
async function selectRuntimeModel(runtime, registry) {
    try {
...
async function checkGroqOnline(runtime) {
    ...
    await vscode.window.withProgress({
        ...
        const client = new groqClient_1.GroqClient({
...
function setMode(agent, chatProvider, mode) {
    agent.setMode(mode);
    chatProvider.postMessage({ type: 'agentEvent', event: { type: 'info', content: `${mode} mode` } });
    chatProvider.sendRuntimeStatus();
}
function sendActiveFileToAgent(agent, chatProvider) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
...
function sendDiagnosticsToAgent(agent, chatProvider) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('No file open');
        return;
...
function updateStatusBar(statusBar, runtime) {
