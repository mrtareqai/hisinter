...
exports.AutonomousRuntimeAgent = void 0;
const executionAdapter_1 = require("./executionAdapter");
const feedbackCollector_1 = require("./feedbackCollector");
const validationOrchestrator_1 = require("./validationOrchestrator");
const learningEngine_1 = require("./learningEngine");
const environmentAdapters_1 = require("./environmentAdapters");
/**
 * AutonomousRuntimeAgent orchestrates the complete execution-feedback-learning loop
 */
class AutonomousRuntimeAgent {
    executionAdapter;
    feedbackCollector;
    ...
    isRunning = false;
    cycleCallbacks = [];
    constructor(toolRegistry, intelligenceOrchestrator, workspacePath = process.cwd()) {
        this.executionAdapter = new executionAdapter_1.ExecutionAdapter(toolRegistry);
        this.feedbackCollector = new feedbackCollector_1.FeedbackCollector(`${workspacePath}/intelligence-feedback`);
        this.validationOrchestrator = new validationOrchestrator_1.ValidationOrchestrator();
        this.learningEngine = new learningEngine_1.LearningEngine(`${workspacePath}/intelligence-learning`);
        this.filesystemMonitor = new environmentAdapters_1.FilesystemMonitor();
        this.terminalExecutor = new environmentAdapters_1.TerminalExecutor();
        this.vscodeAdapter = new environmentAdapters_1.VSCodeAPIAdapter();
     ...
     */
    async executeCycle(taskDescription) {
        ...
        try {
            ...
            const predictions = await this.intelligenceOrchestrator.orchestrate(taskDescription);
            ...
            const executionRecords = await this.executionAdapter.executeSequence(predictions.outputs, predictions.predictions);
            metrics.executionRecords = executionRecords.length;
            // Step 3: Collect feedback from execution
            const feedbacks = await this.feedbackCollector.collectBatchFeedback(executionRecords);
            metrics.feedbackCollected = feedbacks.length;
            // Step 4: Validate collected feedback
            const validations = await this.validationOrchestrator.validateBatch(feedbacks);
            const validFeedbacks = feedbacks.filter((f, i) => validations[i].isValid);
            metrics.feedbackValidated = validFeedbacks.length;
            // Step 5: Learn from validated feedback
            const learningUpdates = await this.learningEngine.processBatchValidatedFeedback(validFeedbacks, validations.filter((v) => v.isValid));
     ...
     */
    async continuousExecutionCycle() {
    ...
    getSystemState() {
        ...
        const avgSuccess = completedCycles.length > 0
     ...
     */
    async executeTerminalCommand(command, onOutput) {
...
exports.AutonomousRuntimeAgent = AutonomousRuntimeAgent;
//# sourceMappingURL=autonomousRuntimeAgent.js.map