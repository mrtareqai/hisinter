...
const RULES = [
    {
        ...
        check: (map) => {
            return map.sourceFiles
                ...
                .map(f => ({
                priority: f.lines > 700 ? 'high' : 'medium',
                area: 'architecture',
    ...
    },
    {
        ...
        name: 'Missing Tests',
        check: (map) => {
            const hasTests = map.sourceFiles.some(f => f.relativePath.includes('test') || f.relativePath.includes('spec'));
            ...
            if (pkgFile) {
                try {
                    ...
                    for (const dep of deps) {
                        const isUsed = map.sourceFiles.some(f => f.content.includes(`'${dep}'`) || f.content.includes(`"${dep}"`));
                        if (!isUsed) {
                            suggestions.push({
                                priority: 'low',
                                area: 'dependencies',
...
class AnalysisPipeline {
    ...
    formatAsMarkdown(result) {
        ...
        if (result.suggestions.length > 0) {
            ...
            for (const s of result.suggestions) {
                ...
                if (s.file) {
                    lines.push(`   📄 File: \`${s.file}\` | ⏱️ ${s.estimatedEffort}`);
                }
                lines.push('');
            }
        }
    ...
    findWeaknesses(map, suggestions) {
        const weaknesses = [];
        const criticals = suggestions.filter(s => s.priority === 'critical').length;
        const highs = suggestions.filter(s => s.priority === 'high').length;
        if (criticals > 0) {
            weaknesses.push(`⚠️ ${criticals} مشاكل حرجة تحتاج إصلاح فوري`);
        }
        if (highs > 0) {
            weaknesses.push(`⚠️ ${highs} مشاكل عالية الأولوية`);
        }
        if (!map.sourceFiles.some(f => f.relativePath.includes('test'))) {
...
//# sourceMappingURL=analysisPipeline.js.map