...
const agentNarrativeEngine_1 = __importDefault(require("../core/agentNarrativeEngine"));
/**
...
class UnifiedExecutionPipeline {
    ...
    pipelineId;
    executionLog;
    constructor(brain, hubIntegration) {
        this.brain = brain;
        this.hubIntegration = hubIntegration;
        this.narrativeEngine = new agentNarrativeEngine_1.default();
     ...
     */
    async executePipeline(userInput) {
        const startTime = Date.now();
        const results = [];
        try {
            // Stage 1: Thinking
            this.logStage('thinking', { input: userInput });
            const thinkingNarrative = this.narrativeEngine.generateThinkingNarrative(userInput);
            ...
            const pipeline = await this.hubIntegration.fullIntelligencePipeline(userInput);
            this.logStage('pipeline-complete', pipeline);
            const decomposedNarrative = this.narrativeEngine.generateDecomposedNarrative(pipeline.decomposition.executionOrder, pipeline.priorities);
            this.narrativeEngine.recordNarrative('decomposed', decomposedNarrative);
            // Stage 4: Validation
            const validation = await this.hubIntegration.validateExecutionPath(pipeline.decomposition);
            this.logStage('validated', validation);
            if (!validation.valid && validation.conflicts.length > 0) {
                // Ask user for clarification
                const questionNarrative = this.narrativeEngine.generateQuestionNarrative('Should I resolve these conflicts automatically or would you prefer to intervene?');
            ...
            for (const goalId of validation.optimizedOrder) {
                ...
                if (!goal)
                    continue;
                const executionNarrative = this.narrativeEngine.generateExecutionNarrative(goal.title, goal.estimatedDuration || 5000);
                this.narrativeEngine.recordNarrative('executing', executionNarrative);
                const executionResult = await this.brain.executeGoal(goalId, goal.description);
                results.push({
                    goalId,
                    goal: goal.title,
                    ...executionResult,
                ...
                const successNarrative = executionResult.success
                    ? this.narrativeEngine.generateSuccessNarrative(goal.title, executionResult.output)
                    : this.narrativeEngine.generateErrorNarrative(executionResult.feedback?.error || 'Unknown error');
                ...
                // Report progress
                const progressNarrative = this.narrativeEngine.generateProgressNarrative(results.length, validation.optimizedOrder.length, executionResult.success ? 'proceeding' : 'recovering');
            ...
            const errorNarrative = this.narrativeEngine.generateErrorNarrative(String(error));
    ...
    async executeGoal(goalId, goalDescription) {
        const narrative = this.narrativeEngine.generateExecutionNarrative(goalDescription, 5000);
     ...
     */
    async *streamExecution(userInput) {
        const startTime = Date.now();
        // Yield thinking
        yield {
            stage: 'thinking',
        ...
        yield {
            ...
            narrative: this.narrativeEngine.generateDecomposedNarrative(pipeline.decomposition.executionOrder, pipeline.priorities),
        };
        // Execute each goal
        const validation = await this.hubIntegration.validateExecutionPath(pipeline.decomposition);
        for (const goalId of validation.optimizedOrder) {
            ...
            yield {
                ...
                narrative: this.narrativeEngine.generateExecutionNarrative(goal.title, goal.estimatedDuration || 5000),
            };
            ...
            yield {
                ...
                narrative: result.success
                    ? this.narrativeEngine.generateSuccessNarrative(goal.title, result.output)
                    : this.narrativeEngine.generateErrorNarrative(result.feedback?.error || 'Unknown error'),
            };
            await this.hubIntegration.learnFromExecution(goalId, {
                success: result.success,
                duration: 0,
