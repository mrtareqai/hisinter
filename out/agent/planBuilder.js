...
class PlanBuilder {
    ...
    buildPlan(analysis, userGoal, projectMap) {
        const phases = [];
        const filesAffected = [];
        const risks = [];
        ...
        const qualitySuggestions = analysis.suggestions.filter(s => s.area === 'quality' || s.area === 'code-quality');
        if (qualitySuggestions.length > 0) {
            phases.push({
                ...
                steps: qualitySuggestions.map(s => ({
                    action: s.area === 'quality' ? 'create' : 'modify',
                    target: s.file || 'tests/',
                    description: s.description,
                })),
                dependencies: [],
